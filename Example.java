			...
			if (isFunction(curr)) stack.push(curr);
				...
				if (curr.equals("(")) stack.push(curr);
					...
					while (!stack.peek().equals("(")) {
						...
						if (stack.isEmpty()) {
							System.out.println("\nНекорректное выражение! Скобки не согласованы.");
							flag = false;
							return postfix;
						}
					}
...
public class Main {
  public static void main(String[] args) {
    ...
    List<String> expressions = expression.parse(s);
    boolean flag = expression.flag;
		if (flag) {
      Ideone ideone = new Ideone();
      Double ideones = ideone.calc(expressions);
      if (ideones.intValue() * 10 == ideones * 10)
        System.out.print("\nОтвет: " + ideones.intValue());
      else
			  System.out.print("\nОтвет: " + ideones);
    }
    in.close();
